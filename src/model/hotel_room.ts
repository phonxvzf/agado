...
const hotelRoomRepo = {
  ...
  getAllReservableHotelRooms: async (hotelId: number): Promise<HotelRoom[]> => {
    return database
      .select('*')
      .from('hotel_room')
      .where('hid', '=', hotelId, 'and', 'reserved', '=', 'FALSE', 'and', 'room_status', '=', 'avail');
  },

  getHotelRoomByRoomNumber: async (hotelId: number, roomNumber: string): Promise<HotelRoom[]> => {
    return database.select('*').from('hotel_room').where('hid', '=', hotelId, 'and', 'rno', '=', roomNumber);
  },

  updateHotelRoom: async (hotelId: number, roomId: number, hotelRoomData: HotelRoom): Promise<number[]> => {
    const completeHotelRoomData = Object.assign({ hid: hotelId, rid: roomId }, hotelRoomData);
    return database('hotel_room').update(completeHotelRoomData).returning('rid').where('hid', '=', hotelId,
      'and', 'rid', '=', roomId);
  },
